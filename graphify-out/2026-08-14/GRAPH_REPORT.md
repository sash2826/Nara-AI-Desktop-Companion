...
1. `cn()` - 124 edges
2. `EmbeddingService` - 63 edges
3. `NullGraphProvider` - 61 edges
4. `FileIndexer` - 58 edges
5. `GraphProvider` - 51 edges
6. `Entity` - 50 edges
...
### Community 0 - "lib.rs"
Cohesion: 0.09
Nodes (106): App, AppHandle, Arc, Child, Client, accept_recommendation(), add_watched_folder(), AddFolderRequest (+98 more)
...
Cohesion: 0.10
Nodes (6): NullGraphProvider, TestGraphQueryService, TestTraversalEngine, provider(), fixture, TestNullGraphProvider
...
Cohesion: 0.11
Nodes (12): Cancel pending debounce timers — called when a watch is removed., Manages the watchdog Observer and the set of watched folders. Persists the…, Load persisted folders and start the watchdog Observer. Safe to call from…, Async start path — used when start() is called from inside the event loop., Stop the Observer and cancel all pending debounce timers., Register a new folder for watching. Triggers an immediate initial index., Persist a folder as watched and start the filesystem watch. Unlike add_folder,…, Unregister a folder and stop watching it. (+4 more)
...
### Community 113 - "SettingsService.ts"
Cohesion: 0.43
